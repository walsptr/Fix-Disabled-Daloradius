...
if (array_key_exists('username', $_GET) && isset($_GET['username']) &&
    ...
    switch ($action) {
        ...
        case 'userEnable':
            ...
            $sql = sprintf("DELETE FROM %s WHERE username IN (%s) AND attribute='%s' ", 
                           $configValues['CONFIG_DB_TBL_RADCHECK'], $username_list, $disabled_attr);
        ...
        case 'userDisable':
            ...
            if (count($to_disable) > 0) {
                ...
                $sql0 = sprintf("INSERT INTO %s (username, attribute, value) VALUES ",
                                $configValues['CONFIG_DB_TBL_RADCHECK']);
                                
                $sql_piece_format = "('%s', '%s', 'Yes')";
                $sql_pieces = array();
                
                foreach ($to_disable as $username) {
                    $sql_pieces[] = sprintf($sql_piece_format, $username, $disabled_attr);
                }
        ...
        case 'checkDisabled':
            ...
            $sql = sprintf("SELECT username FROM %s WHERE username='%s' AND attribute='%s'",
                           $configValues['CONFIG_DB_TBL_RADCHECK'],
                           $dbSocket->escapeSimple($username), $disabled_attr);
            $res = $dbSocket->query($sql);
            $numrows = $res->numRows();
            ...
            if ($numrows > 0) {
                ...
                $message = sprintf('Please note that user <strong>%s</strong> is currently disabled.',
                         ...
                         . '<br>'
                         . sprintf('To enable this user, remove it from the <em>%s</em> profile.', $disabled_attr);
            }
            break;
