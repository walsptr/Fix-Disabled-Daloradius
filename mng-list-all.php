 ...
 */
    ...
    if ($numrows > 0) {
        ...
        if ($per_page_numrows > 0) {
            ...
            $sql = sprintf("SELECT username, attribute FROM %s WHERE username IN (%s)",
                           $configValues['CONFIG_DB_TBL_RADCHECK'], implode(", ", $usernamelist));
            $res = $dbSocket->query($sql);
            $logDebugSQL .= "$sql;\n";
            ...
            while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
                $this_username = $row['username'];
                $this_attribute = $row['attribute'];

                if ($this_attribute === 'Fall-Through') {
                    $records[$this_username]['enabled'] = false;
                } else {
                    array_push($records[$this_username]['groups'],
                               htmlspecialchars($this_attribute, ENT_QUOTES, 'UTF-8'));
                }
            }
        }
